<template>
  <div>
    <div class="block-item" v-for="configItem in qualificationList">
      ...
      <template v-if="configItem.key == 'businessImgs' && CRPlace < 1">
      ...
      <template v-else-if="configItem.key == 'companyRegistrationImgs' && CRPlace >= 1">
...
<script>
  ...
  export default {
    props:['qualificationList','CRPlace'],
    created(){
      this.initView()
    },
    data:()=>({
    ...
    methods:{
      ...
      setList(){
        let list = {
          ...
          companyRegistrationImgs: this.qualificationList[1].list,
          foodCirculationImgs: this.qualificationList[2].list,
        }
...
//        const isJPG = file.type === 'image/jpeg' || file.type === 'image/png';
      ...
      handleFoodCirculationSuccess(res, file) {
        this.addImage(res,2,2)
      },
      handleCompanyRegistrationSuccess(res, file) {
        this.addImage(res,1,3)
      },
      getNotice(msg){
        this.$alert(msg, '提示', {
