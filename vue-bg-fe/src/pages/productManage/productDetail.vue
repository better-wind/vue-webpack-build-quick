        ...
        <div class="item-box ">
                ...
                <template v-else>
                  ...
                  <template v-else-if="item.inputType == 2">
                    <div class="inline-box">
                      ...
                      <el-radio class="mg-b-h" v-model="item.selected" :disabled="item.inputValue ? false : true" :label="item.inputValue"><el-input v-model.trim="item.inputValue" @blur="checkPvName(item,item.inputValue)" class="input-item-middle" size="small"></el-input></el-radio>
                    </div>
                  </template>
                  <!--多选不可自定义-->
                    ...
                    <div class="inline-box">
                      <el-checkbox-group v-model="item.selected">
                        ...
                        <template v-for="(subItem,sIdx) in item.mulOptions">
                          <el-checkbox class="mg-b-h" :disabled="subItem.pvName ? false : true" @change="mulSelect(idx,$event,sIdx)" :label="subItem.pvName"><el-input v-model.trim="subItem.pvName" @blur="checkPvName(item,subItem.pvName,sIdx)" class="input-item-middle" size="small"></el-input></el-checkbox>
                        </template>
                      </el-checkbox-group>
                    </div>
                  </template>
          ...
          <div class="block-box base-box label-float-layout">
            ...
            <div class="inline-box" :style="{'width':'80%'}" v-show="shipAreaOptions.length>0">
              <el-checkbox-group v-model="shipAreaValue" @change="mapValue">
                <el-checkbox v-for="(item,i) in shipAreaOptions" :disabled="!item.isEditable || item.isInStorage == 1 || item.deliverAreaStatus == 0" :key="i" :label="item.deliverAreaId" class="mg-b-h">
          ...
          <template v-for="item in mapShipAreaValue">
            <div class="block-box base-box label-float-layout">
              <!--<div class="label-sign-wrap">-->
                <span class="label-sign"><span class="c-red">*</span>配送地区({{item.deliverAreaName}})</span>
              <!--</div>-->
              <div class="inline-box" >
                <el-select size="small" :disabled="item.isInStorage == 1" :clearable="true" class="mg-b-h" v-model="mapDelTempList[item.deliverAreaId]" placeholder="请选择配送模板">
                  <el-option v-for="subItem in delTempList[item.deliverAreaId]" :key="subItem.id" :label="subItem.name" :value="subItem.id">
                  </el-option>
                </el-select>
              </div>
                      ...
                      <el-checkbox-group v-model="item.selected" @change="mulSelectChange(item.propertyNameId,$event,item)">
                        ...
                        <template v-for="(subItem,sIdx) in item.mulOptions">
                          <el-checkbox class="mg-b-h" :disabled="(subItem.pvName ? false : true) || ((saleUnitDisType == 1 && !item.selected.length && saleUnitDis) || (saleUnitDisType == 2 && saleUnitDis))" @change="mulSelect(idx,$event,sIdx)" :label="subItem.pvName">
                            <el-input v-model.trim="subItem.pvName" :disabled="item.selected.includes(subItem.pvName)" @blur="checkPvName(item,subItem.pvName,sIdx)" class="input-item-middle" size="small"></el-input>
                          </el-checkbox>
                        </template>
                      </el-checkbox-group>
          ...
          <div class="item-table">
            <el-table :data="goodsSkuConfig" style="width: 100%;" >
              ...
              <el-table-column prop="costPrice" align="center" label="供货价(不含邮)" width="120">
                <template slot-scope="scope">
                  <span class="cell-edit-input"><el-input :disabled="(editType == 'edit' && scope.row.isSave && (!isConsult || (isConsult && isExistGoods))) || (saleUnitDisType == 2 && saleUnitDis)" class="input-item-middle" size="small" v-model.trim="scope.row.costPrice"></el-input></span>
                </template>
              </el-table-column>
              <el-table-column prop="code" align="center" :label="'发货编码('+mapArrToObj[item].deliverAreaName+')'" width="120" :key="i" v-for="(item,i) in shipAreaValue" v-if="shipAreaOptions.length>0">
                <template slot-scope="scope">
                  <!--|| mapArrToObj[item].deliverAreaStatus === 0-->
                  <span class="cell-edit-input"><el-input
                      :disabled="(saleUnitDisType == 2 && saleUnitDis) || mapArrToObj[item].isInStorage == 1"
        ...
        <div class="block-box property-box label-float-layout" v-for="item in saleEditModel.propertyList">
          ...
          <div class="inline-box" style="max-width: 600px">
            ...
            <el-radio v-if="!saleEditModel.noKey" class="mg-b-h" v-model="item.selected" :disabled="item.inputValue ? false : true" :label="item.inputValue">
              <el-input v-model.trim="item.inputValue" @blur="checkPvName(item,item.inputValue)" class="input-item-middle" size="small"></el-input>
            </el-radio>
          </div>
        </div>
        <div slot="footer" class="dialog-footer">
...
//          value: 'label',
    ...
    methods:{
      ...
      initView(){
        ...
        this.changeBaseGoodsCode = this.isConsult ? query.baseGoodsCode : this.goodsId
          ...
          this.callGetProductDetail()
            ...
            .then(()=>{
              ...
              this.sellerMainIdChange(function(){
                //展示已选择的发货地
                let _mapDeliverAreas = {}
                _this.goodsDetailFetch.rs.skuArray.map((item)=>{
                  item.deliverAreas.map((area)=>{
                    if(!_mapDeliverAreas[area.deliverAreaId]){
                      _mapDeliverAreas[area.deliverAreaId] = area.deliverAreaId
                ...
                //展示发货地配送模板
                for(let j=0;j<_this.shipAreaOptions.length;j++){
                    let _item = _this.shipAreaOptions[j]

                  if(_this.shipAreaValue.includes(_item.deliverAreaId)){
                    _this.mapShipAreaValue.push(_item)
                  }
                  if(_item.isInStorage == 1){
                    _this.delTempList[_item.deliverAreaId] = [
                      {
                        id:_item.deliverAreaTemplateId,
                        name:_item.deliverAreaTemplateName || ''
                      }
                    ]
                  }
                }
        ...
        this.shipAreaPromise().then((rs)=>{
          ...
          if (this.editType == 'edit') {
            ...
            if (rs.length == 0) {
              ...
              //合并发货地 数据
              rs.map((item,i)=>{
                let _item = this.shipAreaOptions.filter((area)=>item.deliverAreaId == area.deliverAreaId)
                if(_item.length){
                  this.shipAreaOptions.map((area)=>{
                    if(item.deliverAreaId == area.deliverAreaId){

                    }
                })
                } else {
                  this.shipAreaOptions.push(item)
                }
            })
              //合并入仓 数据
              if(this.goodsDetailFetch.rs.skuDeliverAreas && this.goodsDetailFetch.rs.skuDeliverAreas.length){
                this.goodsDetailFetch.rs.skuDeliverAreas.map((item)=>{
                  let _item = this.shipAreaOptions.filter((area)=>item.deliverAreaId == area.deliverAreaId)
                  if(_item.length){
                    this.shipAreaOptions.map((area)=>{
                      if(item.deliverAreaId == area.deliverAreaId){
                        area.isInStorage = item.isInStorage
                        area.deliverAreaStatus = item.deliverAreaStatus
                        if(item.isInStorage){
                          area.deliverAreaTemplateId = item.deliverAreaTemplateId
                          area.deliverAreaTemplateName = item.deliverAreaTemplateName
                        }
                      }
                    })
                  } else {
          ...
          this.shipAreaOptions.map((item)=>{
            if(item.isInStorage == 1){
              this.storageAreaOptions.push(item.deliverAreaId)
            }
          })
          ...
          //接口获取
          let _sellerMainIdMap = []
          this.shipAreaOptions.map((item)=>{
            if(item.deliverAreaId && !_sellerMainIdMap.includes(item.deliverAreaId)){
              _sellerMainIdMap.push(item.deliverAreaId)
            }
          })
          this.sellerDelTempList()
            .then((rs)=>{
              this.delTempList = {}
              _sellerMainIdMap.map((item)=>{
                  if(!this.storageAreaOptions.includes(item)){
                    this.delTempList[item] = rs
                  }
              })
              fn&&fn()
            })
      ...
      selectCate(){
        ...
        if(this.editType == 'edit'){
          this.validCategoryIdChange({baseGoodsCode:this.changeBaseGoodsCode,newCategoryId:_cateId})
            .then((rs)=>{
              if(rs){
            ...
            if(_this.isConsult){
              _this.getBaseGoodsAuditDetail({baseGoodsId:_this.goodsId})
                .then((rs)=>{
                  ...
                  _this.isExistGoods = rs.isExistGoods == 1 ? true : false
...
//        }
      ...
      saveProduct(){
        ...
        opts.deliverAreas = []
        for(let i = 0,j = this.shipAreaValue.length;i<j;i++){
          let item = this.shipAreaValue[i]
          let tempId =  this.mapDelTempList[item]
          if(!tempId){
...
//          }
          ...
          this.shipAreaValue.map((shId)=>{
            if(!this.storageAreaOptions.includes(shId) || !item.isSave || (this.storageAreaOptions.includes(shId) && item['code'+shId])){
              let _opt = {
                deliverAreaId: shId,
          ...
          for(let i=0;i<item.deliverAreas.length;i++){
            if(!(item.deliverAreas[i].deliverCode+'')){
...
//            console.log(this.goodsPropertyConfig[this.configSaleUnitImages.idx].options)
        ...
        if(opts.skuArray[0].properties.length > 2){
          this.getToast('商品限制最多选择2级销售属性')
          return false
        }
//        console.log(opts)
//        debugger
